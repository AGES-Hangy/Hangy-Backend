...
class TagModel(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "(tag_type = 'MACRO' AND tag_parent_id IS NULL) "
            "OR (tag_type = 'MICRO' AND tag_parent_id IS NOT NULL)",
            name="ck_tag_macro_micro_parent",
        ),
        Index("ix_tag_tag_parent_id", "tag_parent_id"),
    )
